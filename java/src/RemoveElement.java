...
public class RemoveElement {
     ...
     *
     * @param nums the array to remove numbers from. Replaces those removed with -1.
     * @param val  the value to remove.
     * @return the number of elements left
     */
    public int removeElement(int[] nums, int val) {
        int empty = -1;
        int totalNumberRemaining = 0;
        ...
        for (int index = 0; index < nums.length; index++) {
            if (nums[index] == val) {
                ...
                nums[freeIndex.poll()] = nums[index];
                nums[index] = empty;
                freeIndex.add(index);
            }
            totalNumberRemaining++;
        }
        return totalNumberRemaining;
    }
}